...
class Member_m extends CI_Model {
	...
	{
		$query = "
			select	/*  > etah_front > member_m > get_member_info_id > ETAH 회원정보를 아이디로 데이터 구하기 */
				  c.CUST_NO
				, c.CUST_LEVEL_CD
				, c.CUST_ID
				, c.CUST_NM
	...
	{
		$query = "
			select	/*  > etah_front > member_m > get_member_info_email > ETAH 회원정보를 이메일로 데이터 구하기 */
				  c.CUST_NO
				, c.CUST_LEVEL_CD
				, c.CUST_ID
				, c.CUST_NM
	...
	{
		 $query = "
			select	/*  > etah_front > member_m > get_member_info_pw1 > ETAH 회원정보를 아이디+비밀번호로 데이터 구하기 */
				  c.CUST_NO
				, c.CUST_LEVEL_CD
				, c.CUST_ID
				, c.CUST_NM
	...
	{
		 $query = "
			select	/*  > etah_front > member_m > get_member_info_pw2 > ETAH 회원정보를 이메일+비밀번호로 데이터 구하기 */
				  c.CUST_NO
				, c.CUST_LEVEL_CD
				, c.CUST_ID
				, c.CUST_NM
	...
	{
		 $query = "
			select	/*  > etah_front > member_m > get_member_info_mobile > ETAH 회원정보를 휴대전화로 데이터 구하기 */
				  c.CUST_NO
				, c.CUST_LEVEL_CD
				, c.CUST_ID
				, c.CUST_NM
    ...
    {
        $query = "
            select	/*  > etah_front > member_m > get_member_info_rcmdID > ETAH 추천인 ID 등록여부 확인하기 */
                  c.CUST_NO
                , c.CUST_LEVEL_CD
                , c.CUST_ID
                , c.CUST_NM
	...
	{
		 $query = "
			insert into	DAT_EMAIL_CERT	(   /*  > etah_front > member_m > regist_email_cert > 이메일 인증 테이블에 쌓기 */
				  EMAIL
				, CERT_NO
				, SEND_DT
				, CERT_STS_CD
	...
	{
		 $query = "
			update	DAT_EMAIL_CERT    /*  > etah_front > member_m > update_email_cert > 이메일 인증 성공/실패 여부 업데이트 */
    ...
    {
        $query = "
			insert into	DAT_MOBILE_CERT	(  /*  > etah_front > member_m > regist_mobile_cert > 휴대폰 인증 테이블에 쌓기 */
				  MOB_NO
				, CERT_NO
				, SEND_DT
				, CERT_STS_CD
    ...
    {
        $query = "
			select	/*  > eroundmall > member_m > get_sms_certkey_chk > sms 인증번호 확인 */
				SC.MOB_NO
    ...
    {
        $query = "
			update	DAT_MOBILE_CERT     /*  > etah_front > member_m > update_mobile_cert > 휴대폰 인증 성공/실패 여부 업데이트 */
			set		  CERT_STS_CD	= '".$sts_cd."'
					, CERT_DT		= '".$cur_date."'
			where
				1 = 1
    ...
    public function insert_mileage_recommendId($cust_id)
    {
        ...
        if( !empty($limit_dt) ){
            if($limit_dt > date('Y-m-d H:i:s')) {   //유효기간 내 회원가입 한 경우
                $query1 = "
				update	DAT_CUST_MILEAGE  m   /*  > etah_front > member_m > insert_mileage_recommendId > 추천인 ID 적립 */
                ...
                $query2 = "
				insert into	DAT_CUST_MILEAGE_SAVING   /*  > etah_front > member_m > insert_mileage_recommendId > 추천인 ID 적립 */
     ...
     *  - 2018.06.01 컬럼추가
     */
    public function regist_member($param, $mem_mobile_no)
    {
        $query = "
			insert into DAT_CUST	(     /*  > etah_front > member_m > regist_member > 회원가입 */
				  CUST_LEVEL_CD
				, CUST_ID
				, PW
				, CUST_NM
	...
	{
		 $query = "
			insert into	LOG_CUST_ACCESS		(    /*  > etah_front > member_m > regist_login_log > 로그인시 로그 기록 쌓기 */
				  CUST_NO
				, ACCESS_DT
			)
			values
	...
	{
		  $query = "
			select      /*  > etah_front > member_m > check_guest_login > 비회원 로그인 */
				o.ORDER_NO
			from
				DAT_ORDER		o
			inner join
	 ...
	 public function regist_member_leave($param)
	{
		...
		$query = "
			insert into DAT_CUST_WITHDRAW(      /*  > etah_front > member_m > regist_member_leave > 회원탈퇴 */
				CUST_NO
				, WITHDRAW_GB_CD";
			if($param['leave_cd'] == '09'){
				$query .= ", ETC_CONTENT";
			}
		$query .= ", USE_YN
			)values(
	 ...
	 public function update_useyn($param)
	{
		...
		$query = "
			update 	DAT_CUST      /*  > etah_front > member_m > update_useyn > 회원탈퇴 */
			set 	USE_YN = 'N'
	 ...
	 public function get_search_member($param)
	{
		 ...
		 $query = "
			select	/*  > etah_front > member_m > get_search_member > ETAH 회원정보 찾기 */
					c.CUST_NO
					, c.CUST_ID
					, c.CUST_NM
    ...
    {
        $query = "
			update	DAT_CUST      /*  > etah_front > member_m > update_temp_password > 임시 비밀번호 세팅 */
    ...
    {
        $query = "
				insert into	DAT_CUST_MILEAGE      /*  > etah_front > member_m > regist_member_mileage > 마일리지 2000지급 */
    ...
    public function regist_mileage($param){
        ...
        $query = "
				insert into	DAT_CUST_MILEAGE_SAVING     /*  > etah_front > member_m > regist_mileage > 마일리지 2000지급 */
    ...
    {
        $query = "
					select R.REG_DT     /*  > etah_front > member_m > get_orderDate > 마일리지 적립 전 주문일시 조회 */
    ...
    public function Email_send_cust($param){
        ...
        $query = "
				INSERT INTO   /*  > etah_front > member_m > Email_send_cust > 이메일 발송 로그 */
    ...
    public function login_sns_naver($param){
        ...
        $query = "  
              select    /*  > etah_front > member_m > login_sns_naver > 간편 로그인 네이버 */
                    s.SNS_NO
                  , s.CUST_NO
                  , s.CUST_ID
                  , s.SNS_ID
    ...
    public function login_sns_kakao($param){
        ...
        $query = "  
              select        /*  > etah_front > member_m > login_sns_kakao > 간편 로그인 카카오 */
                    s.SNS_NO
                  , s.CUST_NO
                  , s.CUST_ID
                  , s.SNS_ID
    ...
    public function get_member_info_sns($men_id){
        ...
        $query = "  
              select  /*  > etah_front > member_m > get_member_info_sns > sns 계정 로그인시 회원조회 */
                  c.CUST_NO
				, c.CUST_LEVEL_CD
				, c.CUST_ID
				, c.CUST_NM
    ...
    {
        $query = "
			insert into DAT_CUST	( /*  > etah_front > member_m > regist_member_s > SNS용 회원가입 */
				  CUST_LEVEL_CD
				, CUST_ID
				, PW
				, CUST_NM
    ...
    public function regist_member_sns($param){
        ...
        $query="
        insert into DAT_CUST_SNS  (CUST_NO,CUST_ID,SNS_ID,SNS_NM,SNS_KIND_CD) /*  > etah_front > member_m > regist_member_sns > sns 테이블 회원추가 */
    ...
    public function member_sns_with($cust_no,$email){
        $query="
            update DAT_CUST c /*  > etah_front > member_m > member_sns_with > 일반회원 sns계정 연동. */
                set c.SNS_YN = 'Y'
                    ,c.EMAIL = ?
            where   c.CUST_NO = ?    
        ";
    ...
    public function update_sns_useyn($param)
    {
        ...
        $query = "
			update 	DAT_CUST_SNS /*  > etah_front > member_m > update_sns_useyn > sns회원탈퇴 */
			set 	USE_YN = 'N'
    ...
    public function sns_gubun()
    {
        ...
        $query = "
			select * from DAT_CUST_SNS s /*  > etah_front > member_m > sns_gubun > sns회원 구별 */
		    where s.CUST_NO = ?
		    and s.USE_YN = 'Y'
		";
    ...
    {
        $query = "
            select * from LOG_SNS_APP_CODE s /*  > etah_front > member_m > isset_log_sns_code > 앱ID_LOG 유무 */
		    where s.ID = ?
        ";
        $db = self::_slave_db();
    ...
    public function log_sns_code($param, $gubun)
    {
        $device = 'W';
        $query = "
            insert into LOG_SNS_APP_CODE(ID, SNS_KIND, DEVICE)  /*  > etah_front > member_m > log_sns_code > 앱ID_LOG */
            values(?, ?, ?)
        ";
    ...
    {
        $query = " 
            select s.CUST_NO  /*  > etah_front > member_m > get_cust_info > sns 회원코드 조회 */
            from DAT_CUST s
            where s.CUST_ID = ?
            and s.USE_YN = 'Y'
            and s.SNS_YN = 'Y'
    ...
    {
        $query = "
			      select  /*  > etah_front > member_m > sns_email_match > 간편 로그인시 이메일과. 기존 회원들 이메일 비교 */
			            * 
			      from DAT_CUST c 
			      where c.EMAIL = '".$email."'
			            and c.USE_YN = 'Y'
    ...
    public function insert_mileage_default($cust_no, $mileage){
        ...
        $query1 = "
				insert into	DAT_CUST_MILEAGE    /*  > etah_front > member_m > insert_mileage_default > 회원가입 마일리지 적립 */
				(CUST_NO,SAVE_MILEAGE_AMT, MILEAGE_AMT, REG_USER_CD)
...
//        log_message("DEBUG", "===========CUST_NO1 = ".$db->last_query());
        $query2 = "
				insert into	DAT_CUST_MILEAGE_SAVING   /*  > etah_front > member_m > insert_mileage_default > 회원가입 마일리지 적립 */
				(CUST_NO, MILEAGE_SAVING_AMT, SAVE_YN,
				 SAVE_DT, SAVING_REASON_GB_CD, SAVING_REASON_ETC, REG_USER_CD)
				values(
...
//        log_message("DEBUG", "===========CUST_NO1 = ".$db->last_query());
        ...
        $query3 = "
				update DAT_CUST     /*  > etah_front > member_m > insert_mileage_default > 회원가입 마일리지 적립 */
...
//        }
    ...
    public function reg_send_sms($type, $param)
    {
        ...
        log_message("DEBUG","===========reg_send_sms start");
        log_message("DEBUG","===========type :". $type);
        log_message("DEBUG","===========param : ".$param);

        if($type == 'SMS'){
            $query = " INSERT INTO uds_msg /* etah_front > member_m > reg_send_sms > SMS발송 ( SMS ) */
        ...
        if($type == 'KAKAO'){
            $query = " INSERT INTO biz_msg /* etah_front > member_m > reg_send_sms > SMS발송 ( KAKAO ) */
							 (MSG_TYPE
								  , CMID
								  , REQUEST_TIME
								  , SEND_TIME
