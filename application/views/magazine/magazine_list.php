    ...
    <div class="nav" id="nav">
        ...
        <ul class="nav_list">
            <?for($i=0;$i<count($nav['CATEGORY_CD1']);$i++) {?>
                <li class="nav_item <?=substr($nav['CATEGORY_CD1'][$i], 0, 1) == substr($category['CATEGORY_CD'], 0, 1) ? "active" : ""?>">
                    <a href="/magazine/mid_list/<?=$nav['CATEGORY_CD1'][$i]?>" class="nav_link"><?=$nav['CATEGORY_NM1'][$i]?></a>
                    <ul class="nav_list_2depth">
                        <?for($j=0;$j<count($nav['CATEGORY_CD2'][$i]);$j++) {?>
                            <li><a href="/magazine/list/<?=$nav['CATEGORY_CD2'][$i][$j]?>" <?=$nav['CATEGORY_CD2'][$i][$j] ==$category['CATEGORY_CD'] ? "class='active'" : ""?>><?=$nav['CATEGORY_NM2'][$i][$j]?></a></li>
                        <?}?>
                    </ul>
                </li>
            <?}?>
        ...
        <div class="position_left">
            <div class="select_wrap select_wrap_cate">
                ...
                <ul id="srp-cate" class="srp-cate1">
                    <?if($category['CATEGORY_CD'] == 90010000) {?>
                    ...
                    <div class="img-wrap auto-img">
                        ...
                        <?if(isset($row['END_DT']) && ($row['END_DT']<date("Y-m-d H:i:s"))){?>
                            <div class="pic_slodout" style="background-color: rgba(0,0,0,0.4);">
                                <?if($this->uri->segment(3) == 90010000) {?>
