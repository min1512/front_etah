...
class Customer_m extends CI_Model {
    ...
    {
        /*
         * 쿼리 성능을 높이기 위한 GROUP_CONCAT
         * 여러줄 데이터 ',' 로 묶어서 한줄로 뽑아냄
         */
    ...
    {
        $query = "
			select 	/*  > etah_front > customer_m > get_notice_list_count > ETAH 공지사항 개수 */
    ...
    {
        $query = "
			select 	/*  > etah_front > customer_m > get_qna_type_list > ETAH 문의 분야 */
					CS_QUE_TYPE_CD
					, CS_QUE_TYPE_CD_NM
					, case 
    ...
    public function get_order_list_by_customer($param)
    {
        ...
        $query = "
			select 	/*  > etah_front > customer_m > get_order_list_by_customer > 1:1문의 작성 고객 주문내역 */
				o.ORDER_NO
				, o.REG_DT
				, r.ORDER_REFER_NO
				, g.GOODS_CD
...
//		var_dump($query);
    ...
    public function get_order_list_count_by_customer($param)
    {
        ...
        $query = "
			select 	/*  > etah_front > customer_m > get_order_list_count_by_customer > 1:1문의 작성 고객 주문내역 개수*/
    ...
    public function get_faq_list($param)
    {
        ...
        $query = "
			select 	/*  > etah_front > customer_m > get_faq_list > FAQ 리스트 */
				f.FAQ_NO
				, f.TITLE
				, f.CS_QUE_TYPE_CD
				, f.REG_DT
...
//		var_dump($query);
    ...
    public function get_faq_list_count($param)
    {
        ...
        $query = "
			select 	/*  > etah_front > customer_m > get_faq_list_count > FAQ 리스트 개수 */
    ...
    public function register_qna($param)
    {
        ...
        $query = "
			insert into	DAT_CS(   /*  > etah_front > customer_m > register_qna > 1:1문의 등록 */
				CS_QUE_GB_CD
				, CS_QUE_TYPE_CD
		 ";
        if($cust_id != 'GUEST'){
...
//		 $cust_no = $this->session->userdata('EMS_U_NO_');
        ...
        $query = "
			insert into	DAT_CS_CONTENTS_REPLY(    /*  > etah_front > customer_m > register_qna_contents > 1:1문의 CONENTS 등록 */
				CS_NO
				, KIND
				, TITLE
				, `CONTENTS`
    ...
    public function register_map_cs_n_order_refer($param)
    {
        ...
        $query = "
			insert into	MAP_CS_N_ORDER_REFER (    /*  > etah_front > customer_m > register_map_cs_n_order_refer > 1:1문의 주문 매핑 */
				CS_NO
				, ORDER_REFER_NO
			)values(
				'".$param['qna_no']."'
    ...
    public function register_map_cs_n_goods($param)
    {
        ...
        $query = "
			insert into MAP_CS_N_GOODS (    /*  > etah_front > customer_m > register_map_cs_n_goods > 1:1문의 상품 매핑 */
				CS_NO
				, GOODS_CD
			)values(
				'".$param['qna_no']."'
...
//		 $cust_no = $this->session->userdata('EMS_U_NO_');
        ...
        $query = "
			insert into MAP_CS_N_ORDER_REFER_CANCEL_RETURN (    /*  > etah_front > customer_m > register_map_cs_n_order_refer_cancel_return > 1:1문의 취소 반품 주문 매핑 */
				CS_NO
				, GOODS_CD
			)values(
				'".$param['qna_no']."'
    ...
    public function update_cs_qna_file_path($title, $qna_no)
    {
        ...
        $query = "
			update	DAT_CS    /*  > etah_front > customer_m > update_cs_qna_file_path > 1:1문의 첨부파일 경로 업데이트 */
			set		FILE_PATH = '".$title."'
    ...
    public function update_qna($param)
    {
        ...
        $query = "
			update	DAT_CS    /*  > etah_front > customer_m > update_qna > 1:1문의 수정 */
    ...
    {
        $query = "
			update	DAT_CS_CONTENTS_REPLY   /*  > etah_front > customer_m > update_qna_content > 1:1문의 수정 CONTENT */
    ...
    public function get_qna_list($param)
    {
        ...
        $query = "
			select	/*  > etah_front > customer_m > get_qna_list > ETAH Q&A 리스트 */
				cs.CS_NO
				, gb.CS_QUE_GB_CD
				, gb.CS_QUE_GB_CD_NM
				, tp.CS_QUE_TYPE_CD
    ...
    public function get_qna_list_count($param)
    {
        ...
        $query = "
			select	/*  > etah_front > customer_m > get_qna_list_count > ETAH Q&A 리스트 개수구하기 */
