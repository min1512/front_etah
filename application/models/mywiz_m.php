...
class Mywiz_m extends CI_Model {
	 ...
	 public function get_coupon_count_by_cust()
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_coupon_count_by_cust > ETAH 고객별 쿠폰 개수*/
	 ...
	 public function get_mileage_by_cust()
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_mileage_by_cust > ETAH 고객별 잔여 마일리지*/
					m.MILEAGE_AMT
	...
	public function regist_qna($param, $gubun)
	{
		if($gubun == 'T'){
			 $query = "
				insert into DAT_CS	(   /*  > etah_front > mywiz_m > regist_qna > ETAH 문의등록*/
					  CS_QUE_GB_CD
					, CS_QUE_TYPE_CD
					, CUST_NO
					, QUE_CUST_NM
			...
			return $rs_identity;
			  ...
			  $query = "
				insert into	DAT_CS_CONTENTS_REPLY	(    /*  > etah_front > mywiz_m > regist_qna > ETAH 문의답변 등록*/
					  CS_NO
					, KIND
					, TITLE
					, CONTENTS
	...
	{
		$query = "
			insert into	MAP_CS_N_GOODS	(   /*  > etah_front > mywiz_m > regist_map_qna_N_goods > ETAH 문의매핑 등록*/
				  CS_NO
				, GOODS_CD
			)
			values
	...
	{
		$query = "
			update 	DAT_CS_CONTENTS_REPLY    /*  > etah_front > mywiz_m > update_goods_qna > ETAH 상품문의 수정*/
	 ...
	 public function get_qna_list($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_qna_list > ETAH Q&A 리스트 */
				cs.CS_NO
				, gb.CS_QUE_GB_CD
				, gb.CS_QUE_GB_CD_NM
				, tp.CS_QUE_TYPE_CD
...
//		var_dump($param['date_type']);
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_qna_list_count > ETAH Q&A 리스트 개수구하기 */
...
//		var_dump($query);
	 ...
	 public function get_qna_detail($qna_no)
	{
		...
		$query = "
			select    /*  > etah_front > mywiz_m > get_qna_detail > ETAH 1:1문의 상세*/
				c.CS_NO
				, c.CS_QUE_GB_CD
				, c.CS_QUE_TYPE_CD
				, c.EMAIL_REPLAY_YN
...
//		var_dump($query);
	 ...
	 public function get_interest_list($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_interest_list > ETAH 관심상품 */
	 ...
	 public function get_interest_list_count()
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_interest_list_count > ETAH 관심상품 개수구하기*/
	...
	{
		 $query = "
			select	/*  > etah_front > mywiz_m > get_wish_list_by_cust_no_n_goods_cd > ETAH 관심상품 체크 */
				 CUST_NO
			from
				DAT_INTEREST_GOODS
			where
	...
	{
		 $query = "
			insert into	DAT_INTEREST_GOODS	(   /*  > etah_front > mywiz_m > register_add_wish_list > ETAH 관심상품 추가 */
				  CUST_NO
				, GOODS_CD
			--	, REG_USER_CD
			--	, UPD_USER_CD
	...
	{
		 $query = "
			update	DAT_INTEREST_GOODS    /*  > etah_front > mywiz_m > update_interest > ETAH 관심상품 삭제 */
			set		USE_YN				= '".$use_yn."'
	 ...
	 public function get_coupon_list($param)
	{
		...
		if($limit_num_rows)	$query_limit = "limit $startPos, $limit_num_rows ";
		//사용 가능한 쿠폰
		$query_date	= "	and c.COUPON_END_DT >= DATE_FORMAT(CURDATE( ), '%Y-%m-%d' )
						and cc.USE_YN = 'Y'";
		//지난 쿠폰
		if($param['last_coupon']) $query_date = "and ( c.COUPON_END_DT < DATE_FORMAT(CURDATE( ), '%Y-%m-%d' ) or cc.USE_YN = 'N') ";

		$cust_no = $this->session->userdata('EMS_U_NO_');

		$query = "
			select	/*  > etah_front > mywiz_m > get_coupon_list > ETAH 쿠폰리스트 */
				cc.COUPON_CD
				, cg.COUPON_GET_CD
				, cg.COUPON_GET_CD_NM
				, cc.COUPON_DTL_NO
...
//		var_dump($query);
	 ...
	 public function get_coupon_list_count($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_coupon_list_count > ETAH 쿠폰리스트 개수구하기*/
	 ...
	 * 쿠폰리스트
	 */
	 public function get_coupon_search_by_goods_code($param)
	{
		$limit_num_rows         = $param['limit_num_rows'];
        $startPos               = ($param['page'] - 1) * $limit_num_rows;
		...
		if($limit_num_rows)	$query_limit = "limit $startPos, $limit_num_rows ";
		//사용 가능한 쿠폰
		$query_date	= "	and	t.COUPON_END_DT >= DATE_FORMAT(CURDATE( ), '%Y-%m-%d' )
						and t.USE_YN = 'Y'";
		//지난 쿠폰
		if($param['last_coupon']) $query_date = "and (t.COUPON_END_DT < DATE_FORMAT(CURDATE( ), '%Y-%m-%d' ) or t.USE_YN = 'N')";

		$cust_no = $this->session->userdata('EMS_U_NO_');

		$query = "
			select      	/*  > etah_front > mywiz_m > get_coupon_search_by_goods_code > ETAH 쿠폰리스트*/
				t.CUST_NO
				, t.DC_COUPON_NM
				, t.MIN_AMT
				, t.MAX_DISCOUNT
				...
				, t.COUPON_CD
			--	, t.COUPON_OBJECT_CD
				, t.BUYER_COUPON_APPLICATION_SCOPE_CD
				, t.USE_YN
			from
			(
				(
					...
					on		cc.COUPON_CD 	= c.COUPON_CD
					and		c.USE_YN		= 'Y'
					and 	c.BUYER_COUPON_APPLICATION_SCOPE_CD = 'GOODS'
					and		c.BUYER_COUPON_GIVE_METHOD_CD = 'CUST_DOWNLOAD'
					inner join	DAT_COUPON_PROGRESS		cp
					on cp.COUPON_PROGRESS_NO		= c.COUPON_PROGRESS_NO
					and cp.COUPON_PROGRESS_STS_CD	= '03'
					and cp.USE_YN					= 'Y'
					inner join	 MAP_COUPON_APPLICATION_SCOPE_OBJECT mg
					on		cc.COUPON_CD	= mg.COUPON_CD
					and		mg.COUPON_APPLICATION_SCOPE_OBJECT_CD = '".$param['goods_cd']."'
				...
				(
					...
					on		cc.COUPON_CD 	= c.COUPON_CD
					and		c.USE_YN		= 'Y'
					and 	c.BUYER_COUPON_APPLICATION_SCOPE_CD = 'BRAND'
					and		c.BUYER_COUPON_GIVE_METHOD_CD = 'CUST_DOWNLOAD'
					inner join	DAT_COUPON_PROGRESS		cp
					on cp.COUPON_PROGRESS_NO		= c.COUPON_PROGRESS_NO
					and cp.COUPON_PROGRESS_STS_CD	= '03'
					and cp.USE_YN					= 'Y'
					inner join	 MAP_COUPON_APPLICATION_SCOPE_OBJECT mb
					on		cc.COUPON_CD	= mb.COUPON_CD
					and		ifnull(mb.COUPON_APPLICATION_SCOPE_OBJECT_CD, '') = (	select	BRAND_CD
																					from	DAT_GOODS
																					where	GOODS_CD = '".$param['goods_cd']."'
																					)
					where	cc.CUST_NO = '".$cust_no."'
				...
				(
					...
					on		cc.COUPON_CD 	= c.COUPON_CD
					and		c.USE_YN		= 'Y'
					and 	c.BUYER_COUPON_APPLICATION_SCOPE_CD = 'CATEGORY'
					and		c.BUYER_COUPON_GIVE_METHOD_CD = 'CUST_DOWNLOAD'
					inner join	DAT_COUPON_PROGRESS		cp
					on cp.COUPON_PROGRESS_NO		= c.COUPON_PROGRESS_NO
					and cp.COUPON_PROGRESS_STS_CD	= '03'
					and cp.USE_YN					= 'Y'
					inner join	 MAP_COUPON_APPLICATION_SCOPE_OBJECT mc
					on		cc.COUPON_CD	= mc.COUPON_CD
					and		mc.COUPON_APPLICATION_SCOPE_OBJECT_CD = (	select	CATEGORY_DISP_CD
																		from	MAP_CATEGORY_DISP_N_GOODS
																		where	GOODS_CD = '".$param['goods_cd']."'
																	)
					where	cc.CUST_NO = '".$cust_no."'
				--	and		cc.USE_YN  = 'Y'
				...
				(
					...
					on		cc.COUPON_CD 	= c.COUPON_CD
					and		c.USE_YN		= 'Y'
					and 	c.BUYER_COUPON_APPLICATION_SCOPE_CD = 'CUST'
					and		c.BUYER_COUPON_GIVE_METHOD_CD = 'CUST_DOWNLOAD'
					inner join	DAT_COUPON_PROGRESS		cp
					on cp.COUPON_PROGRESS_NO		= c.COUPON_PROGRESS_NO
					and cp.COUPON_PROGRESS_STS_CD	= '03'
...
//	var_dump($query);
	 ...
	 * 쿠폰리스트
	 */
	 public function get_coupon_count_search_by_goods_code($param)
	{
		//사용 가능한 쿠폰
		$query_date	= "	and	t.COUPON_END_DT >= DATE_FORMAT(CURDATE( ), '%Y-%m-%d' )
						and t.USE_YN = 'Y'";
		...
		$query = "
			select    /*  > etah_front > mywiz_m > get_coupon_count_search_by_goods_code > ETAH 쿠폰리스트 개수 */
				count(t.CUST_NO)				as total_cnt
	 ...
	 public function get_mileage_list($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_mileage_list > ETAH 마일리지 */
				t.MILEAGE_AMT
				, t.TYPE
				, CUST_NO
	 ...
	 public function get_mileage_list_count($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_mileage_list_count > ETAH 마일리지 개수구하기*/
	...
	{
		 $query = "
			select	/* >etah_front > mywiz_m > get_mileage_info > ETAH 마일리지 정보 */
			--	m.CUST_MILEAGE_NO
				 m.CUST_NO
				, m.MILEAGE_AMT
				, m.SAVE_MILEAGE_AMT
	 ...
	 public function get_delivery_list($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_delivery_list > ETAH 배송지관리 */
	 ...
	 public function get_delivery_list_count($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_delivery_list_count > ETAH 배송지 개수 */
	 ...
	 public function update_base_delivery($base_yn, $deliv_no = null)
	{
		...
		$query = "
			update	DAT_CUST_DELIV_ADDR     /*  > etah_front > mywiz_m > update_base_delivery > ETAH 기본배송지 설정 */
	 ...
	 public function delete_delivery($deliv_no)
	{
		...
		$query = "
			update	DAT_CUST_DELIV_ADDR     /*  > etah_front > mywiz_m > delete_delivery > ETAH 배송지 삭제 */
			set		USE_YN = 'N'
			where	CUST_NO = '".$cust_no."'
	 ...
	 public function register_delivery($param)
	{
		...
		$query = "
			insert into DAT_CUST_DELIV_ADDR     /*  > etah_front > mywiz_m > register_delivery > ETAH 배송지 등록 */
			(
				CUST_NO
				, CUST_DELIV_ADDR_NM
				, RECV_NM
	 ...
	 public function update_delivery($param)
	{
		...
		$query = "
			update	DAT_CUST_DELIV_ADDR     /*  > etah_front > mywiz_m > update_delivery > ETAH 배송지 업데이트 */
	 ...
	 public function get_member_info_by_cust_no()
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_member_info_by_cust_no > ETAH 회원정보 구하기 */
					 c.CUST_ID
					, c.CUST_NM
					, c.MOB_NO
					, c.MOB_REC_YN
	 ...
	 public function get_check_email($email)
	{
		...
		$query = "
			select	c.EMAIL     /*  > etah_front > mywiz_m > get_check_email > ETAH 이메일 중복확인 */
			from	DAT_CUST c
			where	c.EMAIL = '".$email."'
			and		c.CUST_NO <> '".$cust_no."'
...
//		, EMAIL = '".$param['email1']."@".$param['email2']."'

		$query = "
			update 	DAT_CUST       /*  > etah_front > mywiz_m > update_member_info > ETAH 회원 정보 수정 */

			set 	PW 	= PASSWORD('".$param['member_pw']."')
					, MOB_NO = '".$param['mob_phone']."'
					, MOB_REC_YN = '".$param['sns']."'
					, EMAIL = '".$param['hid_email']."'
...
//var_dump($query);
	 ...
	 public function get_order_state_by_cust_no()
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_order_state_by_cust_no > ETAH 주문 상태 정보 구하기 */
				ifnull(sum(if(s.ORDER_REFER_PROC_STS_CD in ('OA00', 'OA01', 'OA02'), '1', '0')),0)	as A /* 입금대기중	*/
				, ifnull(sum(if(s.ORDER_REFER_PROC_STS_CD in ('OA03'), '1', '0')),0)					as B /* 결제완료	*/
				, ifnull(sum(if(s.ORDER_REFER_PROC_STS_CD in ('OB01', 'OB02', 'OB03'), '1', '0')),0)	as C /* 배송준비중	*/
				, ifnull(sum(if(s.ORDER_REFER_PROC_STS_CD = 'OE01', '1', '0')),0)						as D /* 배송중		*/
				, ifnull(sum(if(s.ORDER_REFER_PROC_STS_CD = 'OE02', '1', '0')),0)						as E /* 배송완료	*/
	 ...
	 public function get_order_list($param)
	{
		...
		/* 페이지 구분 */
		switch($param['nav']){
			case 'OA' : $query_cancel_return = "
							\nand rp.ORDER_REFER_PROC_STS_CD in ('OA01', 'OA02','OA03','OB01','OB03','OE01')
							\nand date_add(rp.REG_DT, interval + 7 day) > now()";
							break; //취소/반품/교환신청
		}
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_order_list > ETAH 최근 주문 정보 구하기 */
				o.ORDER_NO
				, r.ORDER_REFER_NO
				, r.ORD_QTY
				, r.GOODS_OPTION_NM
			 ...
			 --	, sum(if(s.ORDER_REFER_PROC_STS_CD in ('OC01', 'OC02' ,'OC21','OC22','OR01','OR02','OR03','OR04','OR11','OR12','OR13','OR21','OR22', 'OB02', 'OB03'), '1', '0'))	as CANCEL_CHANGE_RETURN	/* 취소/교환/반품 */
				, if(rp.ORDER_REFER_PROC_STS_CD in ('OE01', 'OE02'),'Y','N')						as DELIVE_YN
				...
				, vs.SEND_NATION
				, ifnull(cr.QTY, 0) as CANCEL_CHANGE_RETURN
			...
			*/
			from
				...
				inner join	DAT_ORDER_REFER_PROGRESS 	rp
					on	r.ORDER_REFER_NO				= rp.ORDER_REFER_NO
					and r.ORDER_REFER_PROC_STS_NO		= rp.ORDER_REFER_PROC_STS_NO
				left outer join DAT_ORDER_REFER_CANCEL_RETURN cr
					on cr.ORDER_REFER_NO                = r.ORDER_REFER_NO
				inner join	COD_ORDER_REFER_PROC_STS_CD	s
					on	rp.ORDER_REFER_PROC_STS_CD		= s.ORDER_REFER_PROC_STS_CD
				...
				inner join DAT_DELIV_POLICY            p
				    on odf.DELIV_POLICY_NO              = p.DELIV_POLICY_NO
...
//		var_dump($query);
	 ...
	 * 취소, 반품 현황
	 */
	 public function get_order_cancel_return_list($param)
	{
		$limit_num_rows         = $param['limit_num_rows'];
        $startPos               = ($param['page'] - 1) * $limit_num_rows;
		$query_date				= "";
		...
		if($param['nav'] == 'OC'){
			$query_order_state = "and (rp.ORDER_REFER_PROC_STS_CD	like 'OC%'	or	rp.ORDER_REFER_PROC_STS_CD	like 'OR%')";
		}else if($param['nav'] == 'OR'){
			$query_order_state = "and rp.ORDER_REFER_PROC_STS_CD in ('OC21', 'OC22', 'OR21', 'OR22')";
		}

		/* 일자조회 */
		if($param['date_type']>=0) $query_date = "and cr.REG_DT between '".$param['date_from']." 00:00:00' and '".$param['date_to']." 23:59:59'";

		$cust_no = $this->session->userdata('EMS_U_NO_');

		$query = "
			select	/*  > etah_front > mywiz_m > get_order_cancel_return_list > ETAH 취소, 반품 현황 구하기 */
				o.ORDER_NO
				, r.ORDER_REFER_NO
				, r.ORD_QTY
				, substr(r.REG_DT,1,10)							as REG_DT
			...
			and	r.USE_YN = 'Y'
			order by
				cr.ORDER_REFER_CANCEL_RETURN_NO desc
			$query_limit
		";
...
//		var_dump($query);
	 ...
	 * 취소, 반품 현황 개수
	 */
	 public function get_order_cancel_return_list_count($param)
	{
		$query_date				= "";
		$query_order_state		= "";
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_order_cancel_return_list_count > ETAH 취소, 반품 현황 개수 구하기 */
...
//		var_dump($query);
	 ...
	 public function get_order_list_count($param)
	{
		...
		switch($param['nav']){
			case 'OA' : $query_cancel_return = "
							\nand rp.ORDER_REFER_PROC_STS_CD in ('OA01', 'OA02','OA03','OB01','OB03','OE01')
							\nand date_add(rp.REG_DT, interval + 7 day) > now()"; break; //취소/반품/교환신청
		}
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_order_list_count > ETAH 최근 주문 정보 개수 구하기 */
...
//		var_dump($query);
	 ...
	 public function get_order_detail($order_no)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_order_detail > ETAH 주문 상세 정보 구하기 */
				o.ORDER_NO
				, o.REG_DT
				, r.ORD_QTY
				, r.ORDER_REFER_NO
...
//var_dump($query);
	 ...
	 public function get_goods_comment_by_cust($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_goods_comment_by_cust > ETAH 고객 상품평 구하기 */
...
//		var_dump($query);
	 ...
	 public function get_goods_comment_count_by_cust($param)
	{
		...
		$query = "
			select	/*  > etah_front > mywiz_m > get_goods_comment_count_by_cust > ETAH 고객 상품평 개수 구하기 */
...
//		var_dump($query);
     ...
     * 상품평 정보
     */
    public function get_goods_comment_detail($param)
    {
        $query = "
            select    /*  > etah_front > mywiz_m > get_goods_comment_detail > ETAH 고객 상품평 정보 구하기 */
                    cm.CUST_GOODS_COMMENT
                    , `CONTENTS`
                    , GRADE_VAL
                    , substr(cm.GRADE_VAL,1,1) 	as GRADE_VAL1
            ...
            from 
                    DAT_CUST_GOODS_COMMENT cm 
            where 
                    cm.CUST_GOODS_COMMENT = '".$param['comment_no']."'
                    and cm.CUST_NO = '".$param['cust_no']."'
        ";
    ...
    {
        $query = "
            select    /*  > etah_front > mywiz_m > get_goods_comment_file > ETAH 고객 상품평 첨부파일 구하기 */
	...
	{
		 $query = "
			select	/*  > etah_front > mywiz_m > get_cancel_return_reason > ETAH 취소 반품 사유 */
	...
	{
		 $query = "
			select	/*  > etah_front > mywiz_m > get_delivery_company > ETAH 택배업체*/
					DELIV_COMPANY_CD
					, CD_NM
	...
	{
		 $query = "
			select	/*  > etah_front > mywiz_m > get_return_collection_type > ETAH 반품 수거 유형*/
	...
	{
		 $query = "
			select	/*  > etah_front > mywiz_m > get_return_pay_type > ETAH 반품 배송비 결제 방식*/
	 ...
	 public function get_order_refer_detail($order_refer_no)
	{
		...
		$query = "
			select      /*  > etah_front > mywiz_m > get_order_refer_detail > ETAH 주문상세정보 */
				r.ORDER_NO
				, r.ORDER_REFER_NO
				, r.ORD_QTY
				, r.SELLING_PRICE
...
//	 * 주문상세 개수
//	 */
//	 public function get_count_order_refer_by_order_no($page)
//	{
//		$cust_no = $this->session->userdata('EMS_U_NO_');
//
//		$query = "
//			select 	(	select 	count(ORDER_REFER_NO)
//						from	DAT_ORDER_REFER
//						where	ORDER_NO = o.ORDER_NO
//						and		CUST_NO	= '".$cust_no."'
//					)			as CNT
//			from	DAT_ORDER o
//
//			where o.CUST_NO = '".$cust_no."'
//
//			order by
//				o.ORDER_NO desc
//			limit 0, 3
...
//	}
    ...
    {
        $query = "
			update 	DAT_CUST_GOODS_COMMENT      /*  > etah_front > mywiz_m > update_goods_comment > ETAH 상품평 수정 */
    ...
    public function update_goods_comment_file_path($file_path, $file_no)
    {
        ...
        if($file_path == '') {
            $query = "
            update MAP_CUST_GOODS_COMMENT_FILE_PATH f     /*  > etah_front > mywiz_m > update_goods_comment_file_path > ETAH 상품평 첨부파일 삭제 */
            set f.USE_YN = 'N'
        ...
        //첨부파일 변경
            ...
            $query = "
            update MAP_CUST_GOODS_COMMENT_FILE_PATH f     /*  > etah_front > mywiz_m > update_goods_comment_file_path > ETAH 상품평 첨부파일 변경 */
    ...
    {
        $query = "
			update 	DAT_CUST_GOODS_COMMENT      /*  > etah_front > mywiz_m > delete_goods_comment > ETAH 상품평 삭제 */
			set		USE_YN = 'N'
			where	CUST_GOODS_COMMENT = '".$param['comment_cd']."'
        ";
	...
	{
		$query = "
			insert into DAT_CUST_GOODS_COMMENT	(   /*  > etah_front > mywiz_m > regist_comment > ETAH 상품평 등록 */
				  GOODS_CD
				, CUST_NO
				, ORDER_REFER_NO
				, `CONTENTS`
    ...
    public function insert_goods_comment_file_path($title, $goods_comment_no)
    {
        ...
        $query = "
            insert into MAP_CUST_GOODS_COMMENT_FILE_PATH    /*  > etah_front > mywiz_m > insert_goods_comment_file_path > ETAH 상품평 첨부파일 저장 */
            (
                CUST_GOODS_COMMENT
                , FILE_PATH
                , REG_USER_CD
	...
	{
		$query = "
			select    /*  > etah_front > mywiz_m > get_goods_order_refer > ETAH 상품평 상품을 구매했는지 체크 */
				r.ORDER_REFER_NO
			from
				DAT_ORDER		o
			inner join
	...
	{
		$query = "
			select    /*  > etah_front > mywiz_m > get_exists_comment_order > ETAH 상품평을 등록한 적이 있는지 체크 */
    ...
    {
        $query = "
			select    /*  > etah_front > mywiz_m > get_exists_all_comment_order > ETAH 해당 주문상세번호의 상품평 모두 조회 */
				count(gc.CUST_GOODS_COMMENT)	as cnt
			from
				DAT_CUST_GOODS_COMMENT		gc
			where
	...
	{
		$query = "
			update 	DAT_ORDER_REFER_CANCEL_RETURN   /*  > etah_front > mywiz_m > update_cancel_return_ues_yn > ETAH 취반품 사용여부 N */
			set		USE_YN = 'N'
	...
	{
		$query = "
			insert into DAT_ORDER_REFER_CANCEL_RETURN (   /*  > etah_front > mywiz_m > register_order_cancel > ETAH 취소 데이터 생성 */
				CANCEL_RETURN_GB_CD
				, ORDER_REFER_NO
...
//			else	  $attributable_reason_cd = ""; break;
		}

		$query = "
			insert into DAT_ORDER_REFER_CANCEL_RETURN (    /*  > etah_front > mywiz_m > register_order_return > ETAH 반품 데이터 생성 */
				CANCEL_RETURN_GB_CD
				, ORDER_REFER_NO
				, SELLER_CANCEL_RETURN_ORDER_REFER_NO
				, REQ_DT
    ...
    public function register_order_return_file_path($file_path, $return_no)
    {
        ...
        $query = "
            insert into MAP_ORDER_REFER_CANCEL_RETURN_FILE_PATH (    /*  > etah_front > mywiz_m > register_order_return_file_path > ETAH 취소/반품 첨부파일 등록 */
	...
	{
		$query = "
			insert into DAT_ORDER_REFER_PROGRESS (    /*  > etah_front > mywiz_m > register_order_refer_progress > ETAH 주문 상태 생성 */
				ORDER_REFER_NO
	...
	{
		$query = "
			update	DAT_ORDER_REFER   /*  > etah_front > mywiz_m > update_order_refer > ETAH 주문상태 업데이트 */
	...
	{
		$query = "
			update	{$table}   /*  > etah_front > mywiz_m > delete_qna > ETAH 문의 삭제 */
			set		USE_YN = 'N'  
    ...
    public function insert_mileage_comment($param){
        ...
        $query1 = "
				insert into	DAT_CUST_MILEAGE    /*  > etah_front > mywiz_m > insert_mileage_comment > ETAH 상품평 적립 */
        ...
        $query2 = "
				insert into	DAT_CUST_MILEAGE_SAVING     /*  > etah_front > mywiz_m > insert_mileage_comment > ETAH 상품평 적립 */
    ...
    public function get_Orderinfo($param)
    {
        ...
        $query = "select R.ORDER_NO   /*  > etah_front > mywiz_m > get_Orderinfo > ETAH 발송 대상 주문데이터 조회 */
    ...
    {
        $query="    select    /*  > etah_front > mywiz_m > get_payinfo > ETAH 주문 결제정보 */
    ...
    */
    public function get_snsdata()
    {
        ...
        $query="select	/*  > etah_front > mywiz_m > get_snsdata > ETAH 회원정보로 sns 데이터 구하기 */
					 c.CUST_ID
					, c.CUST_NM
					, c.MOB_NO
					, c.MOB_REC_YN
