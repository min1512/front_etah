...
//		$cart = $this->cart_m->get_cart_goods($this->session->userdata('EMS_U_NO_'));
        ...
        if(!empty($row)){
            //장바구니 담긴 상품코드, 카테고리 추출
            foreach($row as $item) {
                $goods[] = $item['GOODS_CD'];
                $category[] = $item['CATEGORY_MNG_CD3'];
            }
            $temp['GOODS'] = implode(', ', $goods);
        ...
        $data['recommend_goods'	] = $this->cart_m->get_cart_best_goods($temp);		//장바구니 추천상품
        $data['ENABLE'          ] = ''; //네이버페이 구매버튼 가능여부
...
//var_dump($goods_item_coupon['COUPON_CD']);
...
//var_dump($data['cart']["2357_4335"][0]);
...
//================================== 판매가 기준 할인 계산법
/////////////////////2016-10-11
//			if($param['goods_coupon_code_s'] != ""){	//상품 셀러쿠폰 할인금액 계산
//				if(explode('||', $param['goods_coupon_code_s'])[1] == 'RATE'){
//					$cart[$i]['SELLER_COUPON_AMT'] = $param['goods_selling_price']* (explode('||', $param['goods_coupon_code_s'])[2]/1000) * $param['goods_cnt'][$i];
//
...
//					}
//				} else {
//					$cart[$i]['SELLER_COUPON_AMT'] = explode('||', $param['goods_coupon_code_s'])[2] * $param['goods_cnt'][$i];
//				}
//			} else {
//				$cart[$i]['SELLER_COUPON_AMT'] = 0;
//			}
//
//			if($param['goods_coupon_code_i'] != ""){	//상품 아이템쿠폰 할인금액 계산
//					if(explode('||', $param['goods_coupon_code_i'])[1] == 'RATE'){
//						$cart[$i]['ITEM_COUPON_AMT'] = $param['goods_selling_price'] * (explode('||', $param['goods_coupon_code_i'])[2]/1000) * $param['goods_cnt'][$i];
//
...
//						}
//					} else {
//						$cart[$i]['ITEM_COUPON_AMT'] = explode('||', $param['goods_coupon_code_i'])[2] * $param['goods_cnt'][$i];
//					}
//			} else {
...
//////////////////////////2016-10-11
//			$cart[$i]['DISCOUNT_PRICE'			] = $cart[$i]['SELLER_COUPON_AMT'] + $cart[$i]['ITEM_COUPON_AMT'];
...
// 배송비 기존 코드
//			if(($param['goods_selling_price'] - $param['goods_discount_price']) * $param['goods_cnt'][$i] < $param['deli_limit'] && $param['deli_limit'] != 0){
...
//					$group_selling_price += ($param['goods_selling_price'] + $param['goods_option_add_price'][$j] - $cart[$i]['DISCOUNT_PRICE'])*$param['goods_cnt'][$j];
                        //2018.09.17 할인가 기준으로 판매가 변경 황승업
...
//				$cart[$i]['COUPON_NUM'				] = $param['goods_coupon_num'			][$i];
                ...
                $cart[$i]['ADD_COUPON_CODE'			] = $param['goods_add_coupon_code'		][$i];
                $cart[$i]['ADD_COUPON_NUM'			] = $param['goods_add_coupon_num'		][$i];
                $cart[$i]['DELIVERY_PRICE'			] = $param['direct_deli_price'		];          // 바로구매 부분 수정.
                $cart[$i]['DELIV_POLICY_NO'			] = $param['deli_policy_no'				][$i];
                $cart[$i]['DELIV_PATTERN_TYPE'		] = $param['deli_pattern'				][$i];
                ...
                $data['cart'][$deli_code."||".$param['direct_deli_price'		]][] = $cart[$i];
        ...
        //var_dump( $param['goods_delivery_price'		][]);
        //      var_dump($param['direct_deli_price'		]);
