...
class Cart_m extends CI_Model {
    ...
    {
        $query = "
			select		/*  > etah_front > cart_m > chk_cart > ETAH 장바구니에 동일 상품&옵션이 담겨있는지 확인 */
				  c.CART_NO
				, c.CART_QTY
				, g.BUY_LIMIT_CD
				, g.BUY_LIMIT_QTY
    ...
    {
        $query = "
			insert into	DAT_CART	( /*  > etah_front > cart_m > add_cart > 장바구니에 상품 담기 */
				CUST_NO
			  , GOODS_CD
			  , GOODS_OPTION_CD
			  , CART_QTY
...
//		";
        ...
        $query = "
			update	DAT_CART    /*  > etah_front > cart_m > del_cart > 장바구니에 상품 제거 */
			set	USE_YN	= 'N'
			where
				1 = 1
			and CART_NO = '".$cart_no."'
    ...
    public function upd_cart($param)
    {
        if($param['gb'] == 'CNT'){
            $query = "
				update	DAT_CART    /*  > etah_front > cart_m > upd_cart > 장바구니에 상품 수량 변경 */
				set
					CART_QTY	= '".$param['cnt']."'
				where
					1 = 1
            ...
            $query = "
				update	DAT_CART    /*  > etah_front > cart_m > upd_cart > 장바구니에 상품 옵션 변경 */
				set
    ...
    {
        $query = "
			select		/*  > etah_front > cart_m > get_cart_goods > ETAH 장바구니에 담은 상품 가져오기 */
				  c.CART_NO
				, c.CUST_NO
				, c.GOODS_CD
				, c.GOODS_OPTION_CD
    ...
    public function get_coupon_info($param, $METHOD)
    {
        if($METHOD == 'AUTO'){	//기본 자동 할인
            $query = "
				select		/*  > etah_front > cart_m > get_coupon_info > 상품에 해당하는 쿠폰 정보 가져오기 */
					  cp.COUPON_CD
					, cp.DC_COUPON_NM
            ...
            $query = "
				select		/*  > etah_front > cart_m > get_coupon_info > 해당 상품의 쿠폰 정보 가져오기 */
					  cp.COUPON_CD
					, cp.CUST_COUPON_NO
					, cp.DC_COUPON_NM
					, ifnull(cp.WEB_DISP_DC_COUPON_NM, cp.DC_COUPON_NM)		as WEB_DISP_DC_COUPON_NM
...
//var_dump($query);
     ...
     * 고객/장바구니 쿠폰 정보 가져오기
     */
    public function get_cust_coupon_info()
    {
        $query = "
			select		/*  > etah_front > cart_m > get_cust_coupon_info > 해당 상품의 고객쿠폰 정보 가져오기 */
					  cp.COUPON_CD
					, cp.DC_COUPON_NM
					, ifnull(cp.WEB_DISP_DC_COUPON_NM, cp.DC_COUPON_NM)		as WEB_DISP_DC_COUPON_NM
					, cp.COUPON_DTL_NO
     ...
     * 장바구니 연관상품 (판매 베스트)
     */
    public function get_cart_best_goods($temp)
    {
        $query_where = "";
        if(!empty($temp)) {
            $query_where = "and g.CATEGORY_MNG_CD in (".$temp['CATEGORY'].")
                                and g.GOODS_CD not in (".$temp['GOODS'].")";
        }

        $query = "
         select     /*  > etah_front > cart_m > get_cart_best_goods > 장바구니 연관상품 */
                 g.GOODS_CD
                , g.GOODS_NM
                , b.BRAND_CD
                , b.BRAND_NM
                ...
                , price.SELLING_PRICE
         from 
                DAT_GOODS_SORT_SCORE rank
                
        inner join 
                DAT_GOODS     g 
        on rank.GOODS_CD    = g.GOODS_CD
        
        inner join 
                DAT_GOODS_PROGRESS    gpro 
        on gpro.GOODS_PROGRESS_NO   = g.GOODS_PROGRESS_NO 
        and gpro.GOODS_STS_CD       = '03'
        ...
        order by 
                rank.GOODS_SORT_SCORE desc, rank.GOODS_CD desc
    ...
    {
        $query = "
			select		/* > etah_front > cart_m > get_post_sido > 우편번호 시/도 가져오기 */
				  zo.SIDO
			from
				DAT_ZIPCODE_OLD		zo
			group by
    ...
    {
        $query = "
			select		/* > etah_front > cart_m > get_post_sigungu > 우편번호 시/군/구 가져오기 */
				  zo.SIGUNGU
			from
				DAT_ZIPCODE_OLD		zo
			where
    ...
    {
        $query = "
			select		/*  > etah_front > cart_m > get_postnum_old > 우편번호 검색 (지번주소) */
				  zo.ZIPCODE
				, zo.SIDO
				, zo.SIGUNGU
				, zo.EUPMYEONDONG
    ...
    public function get_postnum_new($param)
    {
        if($param['gb'] == '02'){
            $query = "
				select		/*  > etah_front > cart_m > get_postnum_new > 우편번호 검색 (도로명주소) */
					  zn.ZIPCODE
					, zn.SIDO
					, zn.SIGUNGU
					, zn.EUPMYEONDONG
    ...
    {
        $query = "
			select		/*  > etah_front > cart_m > get_add_delivery_cost > 도서산간지역 추가배송비 여부 */
				  adc.DELIV_POLICY_NO
    ...
    {
        $query = "
			select		/*  > etah_front > cart_m > get_add_delivery_cost > 배송불가지역 */
				  nd.DELIV_POLICY_NO
				, nd.DELIV_AREA_CD
				, da.DELIV_AREA_NM
				, anz.ZIPCODE
