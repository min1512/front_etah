...
//		 $AccessLog = $this->member_m->regist_login_log($Member);
     ...
     * 회원가입
     */
    public function member_join_post()
    {
        $param = $this->input->post();
        ...
        $data['chk_phone'] = $param['chk_phone'];
    ...
    public function phone_check_post()
    {
        ...
        if( ! empty($row) ) {
            $this->response(array('status'=>'fail', 'message'=>'이미 사용중인 전화번호입니다.'), 200);
        }
        else {
            //휴대폰번호 인증코드 생성
            $tmp_auth	 = "";
            ...
            $mobile_cert = $this->member_m->regist_mobile_cert($mem_mobile, $tmp_authcode, $cur_date);		//휴대폰 인증 테이블에 insert
    ...
    public function mobile_auth_check_post()
    {
        ...
        if (!$result['MOB_NO']) {
            $cur_date = date("Y-m-d H:i:s");
            $email_cert = $this->member_m->update_mobile_cert($param['mem_mobile'], $auth_code, $cur_date, 'ERR');		//휴대폰 인증 update

            $this->response(array('status' => 'fail', 'message'=>'인증번호가 일치하지 않습니다.'), 200);
        } else {
            $cur_date = date("Y-m-d H:i:s");
            $email_cert = $this->member_m->update_mobile_cert($param['mem_mobile'], $auth_code, $cur_date, 'SUC');		//휴대폰 인증 update
    ...
    public function join_post()
    {
        ...
        //적립예정 마일리지 설정
        if($param['mem_birth1'] != '' && isset($param['mem_gender']) && isset($param['petYn']) && isset($param['merry'])) {
        ...
        if( ! empty($exists_member) ) {
            $this->response(array('status'=>'fail', 'message'=>'이미 사용중인 이메일주소입니다.'), 200);
        }
        ...
        $kakao['DEST_PHONE'] = $param['chk_phone'];
...
즐거운 쇼핑정보 만나보세요!";
    ...
    public function background_send_mail_post()
    {
        ...
        $this->email->sendmail($receive, $subject, $body, 'info@etah.co.kr', 'ETAHOME Mall');
    }
...
//		$this->load->view('include/layout');
    ...
    public function ksns_login_get(){
        ...
        $code = $_GET["code"];
        if(!$code){
            echo("<script language=javascript>alert('연동에 실패하였습니다. 다시 시도해주시기 바랍니다.');</script>");
                        ...
                        if($kakao_email == '' || $kakao_email == null){
                            ...
                            if(!empty($cEmail)){
                                ...
                                echo ("<script language=javascript>
                                    ...
                                    if(confirm('회원가입이력이 있습니다. 카카오 연동하시겠습니까?')){
...
//                        log_message('DEBUG','========='.$naver_id);
    ...
    public function sns_join_finsih_post()
    {
        ...
        if( ! empty($exists_member) ) {
            $this->response(array('status'=>'error', 'message'=>'이미 사용중인 휴대폰 번호 입니다. 기존 고객인 경우 1회 로그인을 하신 후 마이페이지 > 간편로그인 연동을 하실 수 있습니다. '), 200);
        }
        ...
        if(empty($cust_no)) {
            $this->response(array('status'=>'error', 'message'=>'System 오류'), 200);
        }
